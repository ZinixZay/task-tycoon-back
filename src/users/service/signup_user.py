...
async def signup_user(user_register_dto: RegisterUserDto) -> EmailStr:
    try:
        user: User = User.create(
            ...
            hashed_password=User.hash_password(user_register_dto.password))
    except Exception as e:
        logger.error(e)
        raise BadRequestException(f'''Не удалось создать пользователя. (Возможно пользователь 
                                  {user_register_dto.email} уже существует)''')
    return user.email
