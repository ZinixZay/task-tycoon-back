...
async def signin_user(response: Response, user_dto: RegisterUserDto = Body(...)) -> None:
    ...
    response.set_cookie(key='ACCESS_TOKEN', value=tokens.ACCESS_TOKEN, samesite='none', secure=True)
    response.set_cookie(key='REFRESH_TOKEN', value=tokens.REFRESH_TOKEN, samesite='none', secure=True)
