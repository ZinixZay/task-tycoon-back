...
async def calculate_summary_attempt_stats(user_id: UUID, task_id: UUID, transaction: Transaction = None) -> SummaryAttemptStats:
    ...
    stats = SummaryAttemptStats(
        ...
        resulting_attempt=await calculate_resulting_attempt(all_attempts, user_id, task_id, transaction)
    )
    return stats
...
async def calculate_resulting_attempt(attemptStatsModels: List[AttemptStatsModel], user_id: UUID, task_id: UUID, transaction: Transaction = None) -> float:
    ...
    if not current_resulting_attempt:
        ...
        if transaction:
            new_attempt_model = AttemptStatsModel(**attempt_stats.to_dict())
            transaction.extend([
                TransactionPayload(
                    method=TransactionMethodsEnum.INSERT,
                    models=[new_attempt_model]
                )])
        ...
        if transaction:
            transaction.extend([
                TransactionPayload(
                    ...
                    models=[AttemptStatsModel(id=current_resulting_attempt.id, stats=stats)]
                )])
        else:
